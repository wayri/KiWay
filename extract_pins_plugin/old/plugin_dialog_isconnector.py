# plugin_dialog.py (Version 2: with both new buttons)
...
class PluginDialog(wx.Dialog):
    ...
    def InitUI(self):
        ...
        # --- NEW BUTTON FOR VERSION 2: Export 'J's ---
        ...
        # --- NEW BUTTON FOR VERSION 2: Export Connectors Only ---
        export_connectors_only_button = wx.Button(panel, label="Export Connectors Only")
        export_connectors_only_button.Bind(wx.EVT_BUTTON, self.OnExportConnectorsOnly)
        button_sizer.Add(export_connectors_only_button, 0, wx.ALL, 5)
        # --- END NEW BUTTONS ---
    ...
    # --- NEW METHOD FOR VERSION 2: Export Connectors Only ---
    def OnExportConnectorsOnly(self, event):
        """
        Event handler for the 'Export Connectors Only' button.
        Filters all board footprints for those with 'isconnector' custom field set to 'yes'.
        """
        print("DEBUG: OnExportConnectorsOnly method called.")

        all_footprints = self.board.GetFootprints()
        connector_footprints = []
        for fp in all_footprints:
            # Check if the footprint has the "isconnector" property
            if fp.HasProperty("isconnector"):
                # Get the value and convert to lowercase for case-insensitive comparison
                prop_value = fp.GetProperty("isconnector").lower()
                if prop_value == "yes":
                    connector_footprints.append(fp)
        
        # Apply sorting if checkbox is checked
        if self.sort_by_reference_checkbox.IsChecked():
            print("DEBUG: Sorting 'isconnector' components by reference.")
            connector_footprints.sort(key=lambda f: f.GetReference())
        else:
            print("DEBUG: Processing 'isconnector' components in board order.")

        if not connector_footprints:
            print("DEBUG: No components with 'isconnector: yes' found on the board.")
            wx.MessageBox("No components with the custom field 'isconnector' set to 'yes' were found.", "No Connectors", wx.OK | wx.INFORMATION)
            return
        
        self._perform_export(connector_footprints, "connectors_only.md", "connectors_only.csv")
        self.EndModal(wx.ID_OK) # Close the dialog after export
    # --- END NEW METHOD ---
